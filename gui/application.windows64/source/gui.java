...
public void quarter_2()
{
  ...
  for(float x = 0.0f; x < period; x += 1/Fs)
  {
  ...
  for(float x = 0.0f; x < period; x += ((float)1/100000))
  {
...
{
  settingsFile.print(PApplet.parseInt(s.A)); settingsFile.print(",");
  settingsFile.print(PApplet.parseInt(s.Fsample)); settingsFile.print(",");
  settingsFile.print(PApplet.parseInt(s.sin_freq)); settingsFile.print(",");
  settingsFile.print(PApplet.parseInt(s.C)); settingsFile.print(",");
...
public float remap(int i)
{
  ...
  output = PApplet.parseInt(output_start + slope * (i -input_start));
...
public void quarter_3()
{
  ...
  for (int i = 1 ; i < lines.length; i++) 
  {
    ...
    ReX[i] = PApplet.parseFloat(list[0]);
    ImX[i] = PApplet.parseFloat(list[1]);
...
public void quarter_4()
{
  ...
  for (int i = 1 ; i < lines.length; i++) 
  {
    line = lines[i];
    String[] list = split(line, '\t');
    t[i] = PApplet.parseFloat(list[0]);
    y[i] = PApplet.parseFloat(list[1]);
    pos.add(t[i], y[i]);
  }
...
public void controlEvent(ControlEvent theEvent) {
  if(theEvent.isAssignableFrom(Textfield.class)) {
    println("controlEvent: accessing a string from controller '"
      ...
      { 
        s.A = PApplet.parseFloat(theEvent.getStringValue());
      ...
      { 
        s.B = PApplet.parseFloat(theEvent.getStringValue());
        update = 1;
        //println(s.A);
      } 
      if(theEvent.getName() == formC) 
      { 
        s.C = PApplet.parseFloat(theEvent.getStringValue());
        update = 1;
        //println(s.A);
      } 
      if(theEvent.getName() == formD) 
      { 
        s.D = PApplet.parseFloat(theEvent.getStringValue());
        update = 1;
        //println(s.A);
      } 
      if(theEvent.getName() == formFsample) 
      { 
        s.Fsample = PApplet.parseFloat(theEvent.getStringValue());
        update = 1;
        //println(s.A);
      } 
      ...
      { 
        s.sin_freq = PApplet.parseFloat(theEvent.getStringValue());
        update = 1;
        //println(s.A);
      } 
      ...
      { 
        s.Nsamples = PApplet.parseInt(theEvent.getStringValue());
        update = 1;
        //println(s.A);
      } 
  }
...
public int inputCheck(String form)
{
  int error = 0;
